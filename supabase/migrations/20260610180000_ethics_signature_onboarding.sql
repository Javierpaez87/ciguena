/*
  ...
  - columnas de referencia en certificates para congelar la firma usada.

  Nota: para el MVP se permite que el worker inserte su propia aceptación.
...
-- ETHICS_CODES
-- ===========================
CREATE TABLE IF NOT EXISTS ethics_codes (
...
ALTER TABLE ethics_codes ENABLE ROW LEVEL SECURITY;

-- ===========================
-- ETHICS_ACCEPTANCES
-- ===========================
CREATE TABLE IF NOT EXISTS ethics_acceptances (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  ethics_code_id uuid NOT NULL REFERENCES ethics_codes(id) ON DELETE RESTRICT,
...
ALTER TABLE ethics_acceptances ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE certificates
  ADD COLUMN IF NOT EXISTS ethics_acceptance_id uuid REFERENCES ethics_acceptances(id),
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'ethics_codes' AND policyname = 'Authenticated can view active ethics codes'
  ) THEN
    CREATE POLICY "Authenticated can view active ethics codes"
      ON ethics_codes FOR SELECT
      TO authenticated
      USING (is_active = true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'ethics_codes' AND policyname = 'Admins can manage ethics codes'
  ) THEN
    CREATE POLICY "Admins can manage ethics codes"
      ON ethics_codes FOR ALL
      TO authenticated
      USING (true)
      WITH CHECK (true);
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'ethics_acceptances' AND policyname = 'Users can view own ethics acceptances'
  ) THEN
    CREATE POLICY "Users can view own ethics acceptances"
      ON ethics_acceptances FOR SELECT
      TO authenticated
      USING (
        user_id IN (SELECT id FROM profiles WHERE auth_user_id = auth.uid())
        OR tenant_id IN (SELECT tenant_id FROM profiles WHERE auth_user_id = auth.uid() AND role IN ('admin', 'super_admin'))
      );
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'ethics_acceptances' AND policyname = 'Workers can insert own ethics acceptance'
  ) THEN
    CREATE POLICY "Workers can insert own ethics acceptance"
      ON ethics_acceptances FOR INSERT
      TO authenticated
      WITH CHECK (
        user_id IN (SELECT id FROM profiles WHERE auth_user_id = auth.uid())
      );
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'Authenticated can upload signature images'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'Authenticated can view signature images'
  ) THEN
...
-- para cada tenant activo. Cuando usemos un Supabase separado por empresa,
-- este seed se ajusta a un solo tenant/brand.
INSERT INTO ethics_codes (tenant_id, title, version, content, content_hash, is_active)
SELECT
  t.id,
...
-- INDEXES
-- ===========================
CREATE INDEX IF NOT EXISTS idx_ethics_codes_tenant_id ON ethics_codes(tenant_id);
CREATE INDEX IF NOT EXISTS idx_ethics_codes_active ON ethics_codes(tenant_id, is_active);
CREATE INDEX IF NOT EXISTS idx_ethics_acceptances_tenant_id ON ethics_acceptances(tenant_id);
CREATE INDEX IF NOT EXISTS idx_ethics_acceptances_user_id ON ethics_acceptances(user_id);
CREATE INDEX IF NOT EXISTS idx_ethics_acceptances_code_id ON ethics_acceptances(ethics_code_id);
